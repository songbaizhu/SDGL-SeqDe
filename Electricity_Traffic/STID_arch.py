...
class STID(nn.Module):
    ...
    def __init__(self, num_nodes, input_len, output_len, num_layer, input_dim=3, node_dim=32, embed_dim=32,
                 ...
                 second_time=7, time_norm=True,patch_len=24):
        super().__init__()
        # attributes
        ...
        self.patch_num = input_len//patch_len

        # spatial embeddings
        if self.if_spatial:
            self.node_emb = nn.Parameter(
                torch.empty(1,1,self.num_nodes, self.node_dim))
        ...
        # 分段
        self.patch_conv = nn.Conv2d(self.hidden_dim,self.hidden_dim,(1,self.patch_len),stride=(1,self.patch_len))
        self.norm = nn.LayerNorm([self.hidden_dim,self.num_nodes,self.patch_num])
        self.encoder = nn.Sequential(
            *[MultiLayerPerceptron(self.hidden_dim, self.hidden_dim) for _ in range(self.num_layer)])
        # module_list = []
        # for _ in range(self.num_layer):
        #     module_list.append(MultiLayerPerceptron(self.hidden_dim, self.hidden_dim))
        #     module_list.append(nn.LayerNorm([self.hidden_dim,self.num_nodes,self.patch_num]))
    ...
    def forward(self, history_data: torch.Tensor, **kwargs):
        ...
        if self.if_time_in_day:
            ...
            if self.time_norm:
                time_in_day_emb = self.time_in_day_emb((
                    t_i_d_data * self.first_time).type(torch.LongTensor).to(input_data.device))
            else:
                time_in_day_emb = self.time_in_day_emb((t_i_d_data).type(torch.LongTensor).to(input_data.device))
        ...
        if self.if_day_in_week:
            ...
            day_in_week_emb = self.day_in_week_emb((
                d_i_w_data).type(torch.LongTensor).to(input_data.device))
        else:
        ...
        if self.if_spatial:
            ...
            node_emb.append(self.node_emb.expand(
                batch_size,T, -1, -1).transpose(1, 3))
        # temporal embeddings
        tem_emb = []
