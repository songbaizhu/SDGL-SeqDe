...
parser.add_argument('--dataset', type=str, default='PEMS04', help='dataset')
# parser.add_argument('--dataset', type=str, default='METR-LA', help='dataset')
parser.add_argument('--data_path', type=str, default=r'D:\文档\PycharmProjects\time_series\raw_data\PEMS04\PEMS04.npz')
# parser.add_argument('--data_path', type=str, default=r'D:\文档\PycharmProjects\time_series\raw_data\METR-LA\METR-LA.h5')
...
parser.add_argument('--weight_decay_mlp', type=float, default=0.0001, help='weight decay rate of STID')
...
# parser.add_argument('--lammda', type=float, default=1, help='静态图与序列相关性的损失比例')
# parser.add_argument('--gamma', type=float, default=0.1, help='稀疏化')
...
from engine_distribution import trainer
...
def main():
    ...
    # load data
    train_dataloader, val_dataloader, test_dataloader, scaler, _, distribute_data,corr_matrix = \
        get_dataloader_stamp(args, 'std', single=False)

    # 相关性矩阵归一化
    # corr_matrix = torch.tensor(corr_matrix, device=device)
    # degree = torch.sum(corr_matrix, dim=1)
    # corr_matrix = corr_matrix / degree.view(-1, 1)
    print('loda dataset done')

    log_string(str(args))
    engine = trainer(scaler,corr_matrix, distribute_data, args)
    ...
    for i in range(1, args.epochs + 1):
        ...
        engine.scheduler1.step(np.mean(valid_rmse))
        engine.scheduler2.step(np.mean(valid_rmse))
    ...
    torch.save(engine.model.state_dict(),
               args.save + "_exp" + "_best_" + ".pth") #args.addaptadj
