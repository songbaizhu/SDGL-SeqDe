...
class gwnet(nn.Module):
    ...
    def __init__(self, device, short_bool, smooth_depth, args, distribute_data=None):
        ...
        self.start_conv2 = nn.Conv2d(in_channels=residual_channels,
                                     out_channels=residual_channels,
        ...
        for j in range(1, layers + 1):
            ...
            self.norm.append(
                LayerNorm((skip_channels, num_nodes, 1),
        ...
        self.skip0 = nn.Conv2d(in_channels=in_dim, out_channels=skip_channels,
                               kernel_size=(1, max(self.seq_length, self.receptive_field*self.patch_len)),
                               bias=True)
        ...
        self.graph_construct = graph_constructor(args.in_dim, self.nodes, args.embed_dim, device, patch_len=self.patch_len)
        ...
        self.mlp_ = nn.Conv2d(in_channels=args.MLP_dim * 4, out_channels=residual_channels, kernel_size=(1, 1),
                              bias=True)
    ...
    def forward(self, input, pred_time_embed=None):
        ...
        if self.patch_num < self.receptive_field:
            x = nn.functional.pad(input, [(self.receptive_field - self.patch_num)*self.patch_len, 0, 0, 0])
        else:
            x = input
        ...
        x = F.relu(self.start_conv1(x))

        x = self.start_conv2(x)        # B,C,N,P
        ...
        for i in range(self.layers):
            ...
            skip = self.norm[i](skip,self.idx)
